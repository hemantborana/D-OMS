...
document.addEventListener('DOMContentLoaded', function() {
   ...
   function categorizeSize(size) {
    ...
    // Extract the numeric part and letter part (if any)
    const numericPart = upperSize.match(/^\d+/)?.[0] || '';
    const letterPart = upperSize.replace(numericPart, '').trim();
    
    // Mapping for letter sizes to categories
    const letterSizeMap = {
        'S': '32',
        'M': '34',
        'L': '36',
    ...
    // If it's a letter size (S, M, L, XL, etc.), get its numeric equivalent
    const effectiveNumeric = letterSizeMap[upperSize] || numericPart;
    
    // Alpha category (32 sizes)
    if (effectiveNumeric === '32') {
        if (['B', 'C', 'D', 'Z', 'DD'].includes(letterPart) || upperSize === 'S') {
            return 'T1';
        }
    }
    // Beta category (34 sizes)
    else if (effectiveNumeric === '34') {
        if (['B', 'C', 'D', 'Z', 'DD'].includes(letterPart) || upperSize === 'M') {
            return 'T2';
        }
    }
    // Gamma category (36 sizes)
    else if (effectiveNumeric === '36') {
        if (['B', 'C', 'D', 'Z', 'DD'].includes(letterPart) || upperSize === 'L') {
            return 'T3';
        }
    }
    // Delta category (38 sizes)
    else if (effectiveNumeric === '38') {
        if (['B', 'C', 'D', 'Z', 'DD'].includes(letterPart) || upperSize === 'XL') {
            return 'T4';
        }
    }
    // Epsilon category (40 sizes)
    else if (effectiveNumeric === '40') {
        if (['B', 'C', 'D', 'Z', 'DD'].includes(letterPart) || upperSize === '2XL' || upperSize === 'XXL') {
            return 'T5';
        }
    }
    // Zeta category (42 sizes)
    else if (effectiveNumeric === '42') {
        if (['B', 'C', 'D', 'Z', 'DD'].includes(letterPart) || upperSize === 'XS') {
            return 'T6';
        }
    }
    
    // For any sizes not in the categories, return as-is
    return size;
}
            ...
            if (item.colors) {
                Object.values(item.colors).forEach(sizeQty => {
                    Object.entries(sizeQty).forEach(([size, qty]) => {
                        ...
                        if (!sizeStats[category]) {
                            sizeStats[category] = 0;
                        }
                        sizeStats[category] += qty;
                    });
                });
            }
        });
    ...
    chartInstances.sizeDistributionChart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                title: {
                    display: true,
                    text: 'Top 10 Size Categories by Demand'
                }
            }
        }
    });
    ...
    chartInstances.allSizesChart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                title: {
                    display: true,
                    text: 'Size Category Distribution Across All Orders'
                }
            },
            scales: {
                y: {
        ...
        order.items?.forEach(item => {
            ...
            if (!productTotals[key]) {
                productTotals[key] = { 
                    ...
                    orderCount: 0,
                    colors: new Set(),
                    sizeCategories: {} // Track size categories per color
                };
            }
            
            // Track original quantities for aggregation
            productTotals[key].orderCount += 1;
            
            if (item.colors) {
                Object.entries(item.colors).forEach(([color, sizeQty]) => {
                    productTotals[key].colors.add(color);
                    
                    // Initialize color entry if not exists
                    if (!productTotals[key].sizeCategories[color]) {
                        productTotals[key].sizeCategories[color] = {};
                    }
                    
                    // Aggregate quantities by size category
                    Object.entries(sizeQty).forEach(([size, qty]) => {
                        const category = categorizeSize(size);
                        if (!productTotals[key].sizeCategories[color][category]) {
                            productTotals[key].sizeCategories[color][category] = 0;
                        }
                        productTotals[key].sizeCategories[color][category] += qty;
                        productTotals[key].totalQty += qty;
                    });
                });
            }
        });
